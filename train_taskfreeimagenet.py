...
def train_online(args):
    ...
    for split_idx in range(args.n_splits):
        ...
        # 【追加】このタスクのデータだけをランダムにシャッフルする
        task_dataset = dataset_part.train_dataset
        indices = list(range(len(task_dataset)))
        random.shuffle(indices)
        shuffled_task_dataset = torch.utils.data.Subset(task_dataset, indices)
        
        # シャッフルしたものをストリームに追加
        stream_datasets.append(shuffled_task_dataset)
    ...
    model.freeze_lang()
    #ema_model = deepcopy(model) #追加ema
    #alpha = 0.9 #追加ema
    ...
    if torch.cuda.is_available():
        ...
        model = model.cuda()
        #ema_model = ema_model.cuda() #追加ema
    else:
    ...
    #scheduler = cosine_lr(optimizer, args.lr, args.warmup_length, 150)
    ...
    for step, batch in enumerate(stream_loader):
        ...
        if is_plateau:
            ...
            #if isinstance(ema_model, torch.nn.DataParallel):
            #    ema_model.module.image_encoder.save(enc_path)
            #    ema_model.module.classification_head.save(head_path)
            #else:
            #    ema_model.image_encoder.save(enc_path)
            #    ema_model.classification_head.save(head_path)
