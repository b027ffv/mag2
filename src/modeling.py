...
class ImageEncoder(torch.nn.Module):
    def __init__(self, args, keep_lang=False):
        super().__init__()
        # timmモデルの読み込み (Online-LoRAと同じ設定)
        # num_classes=0 にすることで特徴量ベクトルを出力するように設定
        self.model = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=0)
        
        # 前処理はtimm/torchvisionのもの手動で定義する必要があります
        # (ここでは簡易的な例です。Online-LoRAのutils/transforms.pyなどを参照して正確に合わせてください)
        self.train_preprocess = transforms.Compose([
            transforms.Resize(224), transforms.ToTensor(), # ... 他に必要な処理
        ])
        self.val_preprocess = transforms.Compose([
    ...
    def freeze_lang(self):
        # モデルが 'transformer' 属性（テキストエンコーダー）を持っている場合のみ凍結処理を行う
        if hasattr(self.model, 'transformer'):
            for pg in self.model.transformer.parameters():
                pg.requires_grad = False
        else:
            # 言語モデルを持たない場合（timmモデルなど）は何もしない
            pass
